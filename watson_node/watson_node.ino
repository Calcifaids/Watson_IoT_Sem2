...
void reconnect() {
  while (!client.connected()) {
    ...
    /*!CHANGE BROKER AND FEEDS TO brain.engineering and /datafeed/... !*/
    //user: admin pass:password123
    if (client.connect("WatsonNode", "admin", "password123")) {
      Serial.println(F("... connected"));

      client.publish("/datafeed/message-log", "Publisher begun");

      /*!ADD FEEDS!*/
      /*!NEED TO KNOW STATES OF ANY CALIBRATION FEEDS!*/
      // Subscribe to relevant feeds
     client.subscribe("/datafeed/recalibrate-ldr");
...
void calibrate_Ldr(uint16_t *maxL, uint16_t *minL){
  ...
  *minL = 1023;
  ...
  client.publish("/datafeed/message-log","LDR recalibrated");
...
void send_Light(uint16_t *maxL, uint16_t *minL){
  if (client.connected()){
    ...
    char feed[] = "/datafeed/light-level";
...
void send_Humidity(){
  if (client.connected()){
    ...
    char feed[] = "/datafeed/humidity-level";
...
void send_Moisture(){
  if (client.connected()){
    ...
    char feed[] = "/datafeed/moisture-level";
